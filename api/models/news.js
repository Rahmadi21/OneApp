var mysql 		= require('mysql');
var uuid  		= require('node-uuid');
var con 		= require('../config/conn.js')
var connection  = mysql.createConnection(con);

module.exports = {
	getNews : function (req, res) {
		var data = {
		"error":1,
		"data":""
	};

	connection.query("SELECT * from tbl_konten WHERE id_kat_konten='e8006ec6-cbbf-11e5-86b1-00269e3e'", function (err, rows, fields){
		if(rows.length != 0){
			data["error"] = 0;
			data["data"] = rows;
			res.json(data);
		}else{
			data["data"] = 'Not Found..';
			res.json(data);
		}
	});

 },

	putNews  : function (req,res){
	var Id = req.body.id;
	var Id_Kat_Konten = req.body.id_kat_konten;
	var Id_User = req.body.id_user;
	var Tgl_Posting = req.body.tgl_posting;
	var Judul = req.body.judul;
	var Isi = req.body.isi;
	var Status = req.body.status;
	var data = {
		"error":1,
		"data":""
	};
	if(Id && Id_Kat_Konten && Id_User && Tgl_Posting && Judul && Isi && Status){
		connection.query("UPDATE tbl_konten SET id_kat_konten=?, id_user=?, tgl_posting=?, judul=?, isi=?, status=? WHERE id=?",[Id_Kat_Konten,Id_User,Tgl_Posting,Judul,Isi,Status,Id],function (err, rows, fields){
			if(!!err){
				data["data"] = err;
			}else{
				data["error"] = 0;
			}
			res.json(data);
		});
	}else{
		data["data"] = "Please provide all required data";
		res.json(data);
	}
},

	deleteNews :function (req, res) {
	var Id = req.body.id;
	var data = {
		"error":1
	};
	if(!!Id){
		connection.query("DELETE FROM tbl_konten WHERE id=?",[Id],function (err, rows, fields){
			if(!!err){
				data["data"] = err;
			}else{
				data["error"] = 0;
				data["data"]  = "hapus sukses";
			}
			res.json(data);
		});
	}else{
		data["data"] = "Please provide all required data";
		res.json(data);
	}}
	,

	postNews : function (req,res){
	var Id = uuid.v4();
	var Id_Kat_Konten = req.body.id_kat_konten;
	var Id_User = req.body.id_user;
	var Tgl_Posting = req.body.tgl_posting;
	var Judul = req.body.judul;
	var Isi = req.body.isi;
	var Status = req.body.status;
	var data = {
		"error":1,
		"data":""
	};
	if(Id && Id_Kat_Konten && Id_User && Tgl_Posting && Judul && Isi && Status){
		connection.query("INSERT INTO tbl_konten VALUES(?,?,?,?,?,?,?)",[Id,Id_Kat_Konten,Id_User,Tgl_Posting,Judul,Isi,Status],function (err, rows, fields){
			if(!!err){
				data["data"] = err;
			}else{
				data["error"] = 0;
			}
			res.json(data);
		});
	}else{
		data["data"] = "Please provide all required data";
		res.json(data);
	}}
}
